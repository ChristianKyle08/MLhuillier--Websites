...
/**
 * A commission is pending for the release screen when:
 * 1) its payment-side commission status is Pending Approval, OR
 * 2) it is already attached to an active RFP (draft/pending_*).
 * The RFP ID is returned so each pending badge opens the correct RFP.
 */
function getCommissionState($status, $amount, $rfp_id = null) {
    ...
    if (strtolower(trim((string)$status)) === 'released') {
        return 'released';
    }
...
try {
    $stmt = $pdo->query("
        SELECT
            ...
            /* One independent RFP ID per commission role. */
            (
                SELECT ri.rfp_id
                FROM rfp_request_items ri
                JOIN rfp_requests rr ON rr.id = ri.rfp_id
                WHERE ri.payment_id = p.id
                  AND ri.commission_role = 'broker'
                  AND rr.status NOT IN ('released', 'rejected')
                ORDER BY ri.id DESC
                LIMIT 1
            ) AS broker_rfp_id,
            (
        ...
        WHERE
            (p.broker_commission_status != 'Released' AND p.broker_commission_amount > 0)
...
<form method="POST" action="/user/encoder/rfp-approval-flow" id="commissionForm">
...
?>
            <form id="rfp_load_<?= $cid ?>_<?= strtotime($date) ?>_<?= $r ?>" method="POST" action="/user/encoder/rfp-approval-flow">

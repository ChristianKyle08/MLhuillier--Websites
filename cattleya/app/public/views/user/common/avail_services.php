...
// Same view-scope rule as the Availed Services dashboard: front-line roles
// (encoder, cashier) only track their own transactions; everyone else sees
// the system-wide total. Keeps this page's pending-count badge consistent
// with what the user will actually see after clicking through.
$role          = $_SESSION['role'] ?? '';
...
try {
    if ($isOwnScope) {
        $stmtCount = $pdo->prepare("SELECT COUNT(*) AS pending_count FROM `avail_services` WHERE `status` = :status AND `process_by` = :process_by");
        $stmtCount->execute(['status' => 'Pending', 'process_by' => $user_session_name]);
    } else {
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            const updateInterval = setInterval(function() {
                ...
                if (elapsedTime >= totalDuration) {
                    ...
                    window.location.href = '/user/common/availed-services';
                }
            }, frameRate);
        });
    </script>

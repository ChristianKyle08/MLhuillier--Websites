    ...
    if ($selected_sale_id) {
        ...
        if ($sale_info) {
            ...
            if (!$is_account_cleared) {
                // Collect all overdue payments first to determine the multiplier
                $overdue_payments = [];
                foreach($payments as $p) {
                ...
                // The multiplier starts at the total number of overdue months
                $multiplier = count($overdue_payments);
                
                // Calculate escalating penalty from top (oldest) to bottom (newest)
                foreach($overdue_payments as $op) {
                    $total_penalty += ($op['amount_due'] * 0.02) * $multiplier;
                    $multiplier--; // Decrease multiplier by 1 for the next month
                }
            }
...
<body class="antialiased">
    <div class="ledger-container only-print">
        ...
        <table class="ledger-table">
            ...
            <tbody>
                <?php 
                // --- Calculate starting multiplier for escalating penalties ---
                $table_overdue_count = 0;
                if (!$is_account_cleared) {
                    foreach ($payments as $p) {
                        $p_status = strtolower($p['status'] ?? '');
                        if (!in_array($p_status, ['paid', 'fully paid']) && $p_status === 'unpaid' && strtotime($p['due_date']) < strtotime(date('Y-m-d'))) {
                            $table_overdue_count++;
                        }
                    }
                }
                $row_multiplier = $table_overdue_count;
                // --------------------------------------------------------------
                ...
                foreach($payments as $row): 
                    ...
                    // Calculate penalty for unpaid rows with escalating multiplier
                    $p_amt = 0;
                    if (!$is_account_cleared && !$is_row_paid && $status_clean === 'unpaid' && strtotime($row['due_date']) < strtotime(date('Y-m-d'))) {
                        $p_amt = ($row['amount_due'] * 0.02) * $row_multiplier;
                        $row_multiplier--; // Decrement multiplier for the next month
                    }
                            ...
                            <div class="bg-white/95 backdrop-blur-md rounded-[2rem] p-6 md:p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all duration-300 border border-slate-100">
                                ...
                                <div class="space-y-4">
                                    <?php 
                                        ...
                                        // --- ADDED: Pre-calculate total overdue months to determine starting multiplier ---
                                        $total_overdue_months = 0;
                                        if (!$is_account_cleared) {
                                            foreach ($payments as $p) {
                                                $p_status = strtolower($p['status'] ?? '');
                                                if ($p_status !== 'paid' && $p_status !== 'fully paid' && $p_status === 'unpaid' && strtotime($p['due_date']) < strtotime(date('Y-m-d'))) {
                                                    $total_overdue_months++;
                                                }
                                            }
                                        }
                                        $current_multiplier = $total_overdue_months;
                                        // ----------------------------------------------------------------------------------
                                        ...
                                        foreach($payments as $row): 
                                            ...
                                            // --- UPDATED: Escalating Penalty Logic ---
                                            $penalty_amt = 0;
                                            if ($is_late) {
                                                $penalty_amt = ($row['amount_due'] * 0.02) * $current_multiplier;
                                                $current_multiplier--; // Decrement the multiplier for the next overdue month
                                            }
                                            // -----------------------------------------
                                                ...
                                                <?php if (!empty($row['ar_number']) || !empty($row['or_number'])): ?>
                                                    <div class="mt-3 flex justify-end gap-2">
                                                        <?php if (!empty($row['ar_number'])): ?>
                                                            <a href="/cattleya/user/fetch/view-receipt?id=<?= $row['id'] ?>&type=AR" target="_blank" 
                                                        ...
                                                        <?php if (!empty($row['or_number'])): ?>
                                                           <a href="/cattleya/user/fetch/view-receipt?id=<?= $row['id'] ?>&type=OR" target="_blank" 
...
async function clearCheck(paymentId, penaltyAmount) {
    ...
    if (isConfirmed) {
        ...
        fetch('/cattleya/user/fetch/process-clear-check', {
            method: 'POST',
        ...
        async function bounceCheck(paymentId) {
            ...
            if (isConfirmed) {
                fetch('/cattleya/user/fetch/process-bounce-check', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
...
async function submitWaiverRequest() {
    ...
    try {
        const response = await fetch('/cattleya/user/fetch/request-waiver', {
            method: 'POST',
...
async function submitFinalPayment() {
    ...
    try {
        const response = await fetch('/cattleya/user/fetch/process-payment', { 
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },

...
$user_role = strtolower($_SESSION['role'] ?? 'encoder');
...
function sortPaymentTerms(&$termsArray) {
    usort($termsArray, function($a, $b) {
        ...
        $a_prio = $priorities[$a] ?? 99;
        $b_prio = $priorities[$b] ?? 99;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // Dynamic router redirect
        header("Location: /user/" . $user_role . "/commission-config?success=1&count=" . $affected_rows);
        exit;
...
try {
    ...
    usort($all_profiles, function($a, $b) {
        ...
        $a_val = $a['release_day'];
        $b_val = $b['release_day'];
        $a_prio = $priorities[$a_val] ?? 99;
        $b_prio = $priorities[$b_val] ?? 99;
                            ...
                            <div class="bg-slate-50/50 rounded-xl p-4 border border-slate-100">
                                ...
                                <div id="dynamic-terms-wrapper" class="terms-container space-y-2 max-h-[320px] overflow-y-auto pr-2">
                                    ...
                                    <div class="grid grid-cols-2 gap-3 items-center group animate-entrance" style="animation-delay: 0.05s">
                                        <?php if($is_custom): ?>
                                            <input type="text" name="custom_terms[]" value="<?= htmlspecialchars($term) ?>" class="p-2.5 bg-white border border-teal-200 rounded-lg text-[11px] font-bold text-teal-700 shadow-sm" readonly>
                                        <?php else: ?>
                                ...
                                <tr class="transition-all duration-200">
                                    <td class="px-4 py-3">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span class="font-bold text-slate-700 text-xs"><?= htmlspecialchars($row['release_day']) ?></span>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-4 py-3">
                                        <span class="px-2 py-0.5 rounded text-[9px] font-black bg-slate-100 text-slate-600 border border-slate-200"><?= htmlspecialchars($row['duration']) ?></span>
                                    </td>
                                    <td class="px-4 py-3">
                                        <span class="text-xs font-bold text-teal-600">Month <?= htmlspecialchars($row['payment_division']) ?></span>
                                    </td>

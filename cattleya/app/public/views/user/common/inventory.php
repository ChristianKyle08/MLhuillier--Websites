...
<head>
    ...
    <style>
    :root {
        /* Brand palette — unified to match the teal/lime identity already used
           throughout the page, so every var() below resolves to a real value. */
        --primary-color: #2a6279;
        --primary-hover: #214e61;
        --primary: #2a6279;
        --primary-dark: #1e4a5c;
        --primary-light: #cfe0e6;
        --accent: #96c93d;
        --accent-dark: #7eb02a;
        --bg: #f8fafc;
        --bg-soft: #f8fafc;
        ...
        --shadow-lg: 0 20px 32px -12px rgba(15, 23, 42, 0.14), 0 8px 12px -6px rgba(15, 23, 42, 0.06);
    ...
    * { -webkit-font-smoothing: antialiased; -moz-osx-font-smoothing: grayscale; }
    ...
    body {
        ...
        background-image:
            radial-gradient(at 0% 0%, rgba(42, 98, 121, 0.05) 0px, transparent 50%),
            radial-gradient(at 100% 0%, rgba(150, 201, 61, 0.04) 0px, transparent 45%);
        background-attachment: fixed;
        min-height: 100vh;
    }

    /* Slim, unobtrusive scrollbars for a more polished feel (progressive enhancement) */
    ::-webkit-scrollbar { width: 8px; height: 8px; }
    ::-webkit-scrollbar-track { background: transparent; }
    ::-webkit-scrollbar-thumb { background: #cbd5e1; border-radius: 8px; }
    ::-webkit-scrollbar-thumb:hover { background: #94a3b8; }

    .main-content { margin-left: 15px; padding: 3rem; transition: padding 0.3s var(--ease-smooth); }
    @media (max-width: 991px) { .main-content { margin-left: 0; padding: 1.5rem; } }

    .header-section h2 { font-weight: 800; letter-spacing: -0.03em; color: var(--text-main); }
    ...
    .project-icon-wrapper::after {
        ...
        width: 100%;
        height: 100%;
        background: rgba(42, 98, 121, 0.88);
        color: white;
        font-size: 0.6rem;
        font-weight: 800;
    ...
    .status-mini-card:hover { transform: translateY(-2px); border-color: var(--border-color); }

    .text-avail { color: #00b894; }
    .text-res { color: #3b82f6; }
    ...
    /* ============ LOT TILE (shared between card badges + inventory map) ============ */
    .lot-tile {
        position: relative;
        overflow: hidden;
        padding: 8px 12px;
    ...
    .lot-tile.status-available::after { background: rgba(4, 120, 87, 0.92); }
    .lot-tile.status-sold::after { background: rgba(190, 18, 60, 0.92); }
    .lot-tile.status-reserved::after { background: rgba(55, 48, 163, 0.92); }
    .lot-tile.status-inactive::after { background: rgba(71, 85, 105, 0.92); }
    ...
    .lot-tile:hover::after { bottom: 0; }

    .lot-tile .lot-number-text { transition: opacity 0.3s ease; }
    .lot-tile:hover .lot-number-text { opacity: 0.2; }

    /* Standalone status swatches (reserved for badges / future use) */
    .status-available { background: #ecfdf5; color: #065f46; border-color: #d1fae5; }
    .status-reserved { background: #eff6ff; color: #1e40af; border-color: #dbeafe; }
    ...
    .status-card .dot { width: 8px; height: 8px; border-radius: 50%; background: #cbd5e1; transition: box-shadow 0.25s ease; }
    ...
    /* ============ SEGMENTED CONTROL (New / Existing customer) ============ */
    .segmented-control {
        display: flex;
        background: #f1f5f9;
        padding: 4px;
    ...
    /* ============ FORM FIELDS ============ */
    .modern-field { position: relative; margin-bottom: 12px; }
    .modern-field i {
        position: absolute;
        left: 12px;
        top: 50%;
    ...
    .modern-field .form-control:focus {
        ...
        box-shadow: 0 0 0 3px rgba(42, 98, 121, 0.12);
    }
    ...
    .client-info-item { display: flex; align-items: flex-start; gap: 10px; margin-bottom: 8px; font-size: 0.85rem; color: #475569; }
    .client-info-item i { color: #94a3b8; margin-top: 3px; }

    .swal2-confirm, .swal2-cancel { transition: transform 0.2s ease, box-shadow 0.2s ease !important; }
    .swal2-confirm:hover {
        transform: translateY(-1px);
    ...
    .insta-note {
        ...
        white-space: nowrap;
        pointer-events: none;
        /* Plays twice on load to catch the eye, then rests — avoids an
           endless loop running on every single product card at once. */
        animation: floatNote 2.4s ease-in-out 2;
    }
    .insta-note::after {
        content: '';
        position: absolute;
    ...
    /* Matches the 28–32px modal radius used across SweetAlert2 popups */
    .premium-modal-radius {
        border-radius: 32px !important;
        font-family: 'Plus Jakarta Sans', sans-serif !important;
    }
    .swal2-timer-progress-bar { background: #2a6279 !important; }
    ...
    .form-select:focus, .form-control:focus {
        ...
        box-shadow: 0 0 0 4px rgba(42, 98, 121, 0.12) !important;
        outline: none;
    }
    ...
    .payment-input { border-radius: 0 12px 12px 0 !important; }
    ...
    @keyframes highlight {
        0% { background-color: rgba(42, 98, 121, 0.12); }
        100% { background-color: transparent; }
    }
    .value-updated { animation: highlight 1s ease-out; }
</style>
</head>
...
<div class="main-content">
    ...
    <script>
    // Defined once (outside the product loop above) since it's identical for every
    // card and takes no arguments — no need to redeclare it per product.
...
async function saveImage(productId) {
    ...
    try {
        const response = await fetch('/cattleya/user/fetch/upload-photo', {
            method: 'POST',
            body: formData
        });
...
async function openViewModal(productName) {
    ...
    try {
        const res = await fetch(`/cattleya/user/fetch/get-product-details?name=${encodeURIComponent(productName)}`);
        ...
        container.innerHTML = `
            <style>
                ...
                .status-dot {
                    ...
                    animation: statusPulse 2.5s ease-in-out 3;
                }
                        ...
                        ${groups[block].map((l, lotIndex) => {
                            ...
                            return `
                                <div class="lot-wrapper lot-item-${block}" data-lot="${l.lot_number}" style="width: max-content; flex: 0 0 auto; display: flex; flex-direction: column; align-items: center; gap: 6px; animation-delay: ${(lotIndex * 0.02)}s;">
                                    ...
                                    <div class="lot-tile premium-lot-tile status-${l.status} ${isInactive ? 'inactive-tile' : ''}" 
                                        ${clickEvent}
                                        ${titleAttr}
...
async function generateNextCustomerId() {
    ...
    try {
        // FIXED URL: The .php extension is now actually included in the string
        const response = await fetch('/cattleya/user/fetch/get-next-customer-id'); 
        ...
        if (result.success) {
            ...
            custInput.value = "CUST-" + new Date().getFullYear() + "-FAILED";
        }
    } catch (e) {
        console.error("JavaScript Fetch Error:", e);
        custInput.value = "CUST-" + new Date().getFullYear() + "-ERROR";
    }
}
...
async function triggerStatusUpdate(id, prodName, block, lot, niche, block_description, lot_price, tcp, cash_price, currentStatus) {
    ...
    if (isOccupied) {
        ...
        try {
            const url = `/cattleya/user/fetch/get-sale-details?block=${block}&lot=${lot}&product=${encodeURIComponent(prodName)}`;
...
document.getElementById('btnMarkAsSold').addEventListener('click', async function() {
    ...
    if (confirm.isConfirmed) {
        ...
        try {
            ...
            const res = await fetch("/cattleya/user/fetch/update-sale-status", {
                method: "POST",
                body: formData
            });
...
document.getElementById('cancelReservationForm').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        const res = await fetch("/cattleya/user/fetch/cancel-reservation", {
            method: "POST",
            body: formData 
        });
    ...
    async function populateBlocks(productId, selectedBlock = null) {
        ...
        if (!productId) return;
        try {
            const res = await fetch(`/cattleya/user/fetch/get-blocks?product_id=${productId}`);
...
document.getElementById('updateStatusForm')?.addEventListener('submit', async function(e) {
    ...
    try {
        ...
        const res = await fetch("/cattleya/user/fetch/add-sales", {
            method: "POST",
            body: formData
        });
        const data = await res.json();
...
async function openAddForm(productName) {
    ...
    try {
        const response = await fetch(`/cattleya/user/fetch/get-suggestions?product_name=${encodeURIComponent(productName)}`);
...
addLotForm.addEventListener('input', async function(e) {
    ...
    // ADDED CONDITION: Prevent zeroing or overwriting TCP when typing into TCP or Cash Price fields, and vice versa
    const isTcpField = target.name === 'main_tcp' || target.id === 'main_tcp' || target.id === 'raw_price' || target.id === 'disp_price';
    const isCashField = target.name === 'cash_price' || target.id === 'cash_price' || target.id === 'raw_cash_price' || target.id === 'disp_cash_price';

    if (!isTcpField && !isCashField) {
        ...
        // Preserve TCP value if price breakdown fields are empty/0 and TCP already holds a value
        if (total > 0) {
            this.main_tcp.value = total.toFixed(2);
        } else if (!this.main_tcp.value || (price === 0 && vat === 0 && careFund === 0 && marketing === 0)) {
            // Only clear TCP if all breakdown components are explicitly empty or 0 and TCP is not being preserved
            if (this.lot_price?.value === '' && this.vat?.value === '' && this.care_fund?.value === '' && this.marketing_budget?.value === '') {
                this.main_tcp.value = '';
            }
        }
    }
    ...
    if (target.name === 'block_number') {
        ...
        if (blockVal.trim() !== "") {
            try {
                const res = await fetch(`/cattleya/user/fetch/get-suggestions?product_name=${encodeURIComponent(productName)}&block_number=${encodeURIComponent(blockVal)}`);
                const data = await res.json();
                if (data.success) {
...
addLotForm.addEventListener('submit', async function(e) {
    ...
    try {
        const response = await fetch('/cattleya/user/fetch/save-lot', { 
            method: 'POST',
            body: formData
        });
...
async function loadCustomerList() {
    try {
        const res = await fetch('/cattleya/user/fetch/get-customers?action=list');
...
document.getElementById('sel_existing_customer').addEventListener('change', async function() {
    ...
    try {
        const res = await fetch(`/cattleya/user/fetch/get-customers?action=details&id=${customerId}`);
        const result = await res.json();
        const c = result.data;

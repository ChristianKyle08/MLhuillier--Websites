...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && in_array($flow_action, ['generate_flexible_rfp', 'load_existing_rfp', 'load_multiple_rfps', 'group_multiple_rfps'], true)) {
    ...
    if ($flow_action === 'generate_flexible_rfp') {
        ...
        if (count($rfp_ids) < 2) {
            header('Location: /user/' . $logged_in_role . '/commission-release?error=' . urlencode('Select at least two RFP requests to group under one requisition slip.'));
            exit;
        }
        ...
        try {
            ...
            header('Location: /user/' . $logged_in_role . '/commission-release?success=' . urlencode(count($rfp_ids) . ' RFP requests were grouped under requisition slip ' . $master_slip . '.'));
            exit;
        } catch (PDOException $e) {
            header('Location: /user/' . $logged_in_role . '/commission-release?error=' . urlencode('Failed to group the selected RFP requests.'));
            exit;
        }
    }
}
                        ...
                        try {
                            ...
                            foreach ($rfp_data['selected_keys'] as $key) {
                                ...
                                if (!empty($payment_id)) {
                                    ...
                                    // ADDED CONDITION: Separates the charge value in every personnel dynamically within the single payments table row
                                    $charge_col = $role . '_charge'; // Maps to agent_charge, broker_charge, or um_charge

                                    // ADDED CONDITION: only write {role}_charge if that column actually
                                    // exists on `payments`. The base schema ships {role}_commission_status
                                    // but not {role}_charge yet, so writing it unconditionally throws
                                    // "column not found" and rolls back the whole submission the moment
                                    // any charge is entered. This keeps broker/um/agent charges separated
                                    // per personnel (still one row each in rfp_request_items below,
                                    // unchanged) while staying safe on databases that haven't added the
                                    // {role}_charge columns yet. To also cache the separated charge
                                    // directly on this payments row, run once:
                                    //   ALTER TABLE payments
                                    //     ADD COLUMN agent_charge  DECIMAL(10,2) DEFAULT NULL,
                                    //     ADD COLUMN broker_charge DECIMAL(10,2) DEFAULT NULL,
                                    //     ADD COLUMN um_charge     DECIMAL(10,2) DEFAULT NULL;
                                    // After that, this same code starts populating them automatically -
                                    // no further changes needed here.
                                    static $paymentsChargeColumnExists = [];
                                    if (!array_key_exists($charge_col, $paymentsChargeColumnExists)) {
                                        $colCheckStmt = $pdo->prepare("SHOW COLUMNS FROM payments LIKE ?");
                                        $colCheckStmt->execute([$charge_col]);
                                        $paymentsChargeColumnExists[$charge_col] = (bool) $colCheckStmt->fetch();
                                    }

                                    if ($paymentsChargeColumnExists[$charge_col]) {
                                        $stmt = $pdo->prepare("UPDATE payments SET `{$status_col}` = 'Pending Approval', `{$charge_col}` = ? WHERE id = ? LIMIT 1");
                                        $stmt->execute([$item_charge, $payment_id]);
                                    } else {
...
if (!$rfp): ?>
    <div style="min-height: 100vh; display: flex; align-items: center; justify-content: center; font-family: system-ui, -apple-system, sans-serif; background: linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%);">
        <div style="text-align: center; background: #fff; padding: 48px 56px; border-radius: 20px; box-shadow: 0 20px 40px -10px rgba(0,0,0,0.08); border: 1px solid #e2e8f0; max-width: 440px;">
            ...
            <a href="/cattleya/user/<?= htmlspecialchars($logged_in_role ?? '') ?>/commission-release" style="display: inline-block; width: 100%; padding: 14px; background: #0f766e; color: #fff; border-radius: 12px; font-weight: 600; text-decoration: none; font-size: 15px; transition: background 0.2s ease;">Return to Commissions</a>
        </div>
    </div>
    <?php exit;
endif;
...
// ADDED CONDITION: figure out, across the WHOLE item set, which payment_id
// values are shared by more than one row (e.g. both a UM commission and an
// Agent commission owed against the same underlying payment). Each item's
// own $roles_to_expand below is built from a single already-role-specific
// selected_key, so it is almost always length 1 and can never see this by
// itself - checking count($roles_to_expand) > 1 was checking the wrong
// scope, which is why same-payment UM/Agent/Broker charges were colliding
// on one shared "charges[<payment_id>]" input and overwriting each other.
$paymentIdOccurrences = [];
if (!empty($rfp['items'])) {
    foreach ($rfp['items'] as $probe_item) {
        $probe_payment_id = $probe_item['id'] ?? $probe_item['payment_id'] ?? null;
        if ($probe_payment_id !== null && $probe_payment_id !== '') {
            $paymentIdOccurrences[$probe_payment_id] = ($paymentIdOccurrences[$probe_payment_id] ?? 0) + 1;
        }
    }
}
...
if (!empty($rfp['items'])) {
    foreach ($rfp['items'] as $idx => $item) {
        ...
        foreach ($roles_to_expand as $role) {
            ...
            // ADDED CONDITION: key on how many rows across the whole set
            // share this payment_id (see $paymentIdOccurrences above), not
            // on this single item's own role count, which is what let
            // same-payment UM/Agent/Broker charges share one input name
            // and stomp each other's values.
            $shares_payment_id = $payment_id !== null && $payment_id !== ''
                && ($paymentIdOccurrences[$payment_id] ?? 0) > 1;
            $unique_input_key = $shares_payment_id ? $payment_id . '_' . $role : $payment_id;
        ...
        <form id="rfp-workflow-form" method="POST" enctype="multipart/form-data">
            ...
            <div class="cp-actions-bar">
                <div class="action-group-left">
                    <a href="/user/<?= htmlspecialchars($logged_in_role) ?>/commission-release" class="btn btn-outline">

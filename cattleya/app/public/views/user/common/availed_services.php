...
// Front-line roles (encoder, cashier) only manage the transactions they
// personally processed. Oversight roles (admin, auditor, finance, cfo, vpo,
// operation_manager) keep full system-wide visibility of the ledger.
$user_name     = $_SESSION['user_name'] ?? 'System Administrator';
$role          = $_SESSION['role'] ?? '';
$ownScopeRoles = ['encoder', 'cashier'];
$isOwnScope    = in_array($role, $ownScopeRoles, true);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_status_submit'])) {
    try {
        ...
        // Restricted roles may only settle records that carry their own name in
        // process_by — this mirrors the scoping used for the table below so the
        // action can't be triggered on someone else's transaction via a raw POST.
        if ($isOwnScope) {
            $stmtUpdate = $pdo->prepare("UPDATE avail_services SET status = ?, amount_paid = ?, change_amount = ? WHERE id = ? AND process_by = ?");
            $stmtUpdate->execute([$new_status, $amount_paid, $change_amount, $record_id, $user_name]);
        } else {
            $stmtUpdate = $pdo->prepare("UPDATE avail_services SET status = ?, amount_paid = ?, change_amount = ? WHERE id = ?");
            $stmtUpdate->execute([$new_status, $amount_paid, $change_amount, $record_id]);
        }
        ...
        if ($stmtUpdate->rowCount() > 0) {
            ...
            throw new Exception("Unable to update this record — it may not exist, or it isn't assigned to you.");
        }
    } catch (Exception $e) {
...
try {
    if ($isOwnScope) {
        $stmt = $pdo->prepare("SELECT * FROM avail_services WHERE process_by = ? ORDER BY date_avail DESC");
    ...
    // Calculate statistics for the informative dashboard (already scoped above,
    // so these totals always match what the current user is allowed to see)
        ...
        <header class="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-slate-200 pb-4">
            <div>
                <div class="flex flex-wrap items-center gap-2 mb-2">
                    <span class="px-2.5 py-1 bg-brand-primary/10 text-brand-primary rounded-md text-[10px] font-bold uppercase tracking-widest inline-block">Ledger Tracking</span>
                    <span class="px-2.5 py-1 <?= $isOwnScope ? 'bg-amber-50 text-amber-700 border border-amber-200' : 'bg-emerald-50 text-emerald-700 border border-emerald-200' ?> rounded-md text-[10px] font-bold uppercase tracking-widest inline-flex items-center gap-1.5">
                        <i class="fa-solid <?= $isOwnScope ? 'fa-user-lock' : 'fa-globe' ?>"></i>
                        <?= $isOwnScope ? 'My Transactions Only' : 'All Transactions · System-wide' ?>
                    </span>
                </div>
                ...
                <p class="text-slate-500 text-sm mt-1 max-w-2xl leading-relaxed">
                    Signed in as <span class="font-bold text-slate-700"><?= htmlspecialchars($user_name) ?></span>
                    <span class="text-slate-300">&middot;</span>
                    <span class="font-semibold text-brand-primary"><?= htmlspecialchars(ucwords(str_replace('_', ' ', $role))) ?></span>
                </p>
            </div>
            
            <a href="/user/common/avail-services" class="inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-gradient-to-r from-brand-primary to-brand-dark text-white text-xs text-decoration-none font-bold uppercase tracking-wider rounded-lg hover:shadow-lg hover:shadow-brand-primary/30 transform hover:-translate-y-0.5 transition-all duration-200 focus:ring-2 focus:ring-offset-2 focus:ring-brand-primary">
                            ...
                            ?>
                                <tr class="hover:bg-slate-50 transition-colors group data-row">
                                    <td class="px-5 py-3.5 pl-6">
                                        ...
                                        <?php if (!$isOwnScope): ?>
                                            <p class="text-[10px] text-slate-400 mt-0.5"><i class="fa-solid fa-user-pen text-slate-300 mr-1"></i> <?= htmlspecialchars($record['process_by'] ?? 'N/A') ?></p>
                                        <?php endif; ?>
                                    </td>
                        ...
                        <?php if(count($availed_records) === 0): ?>
                            <tr id="emptyStateRow">
                                <td colspan="6" class="px-4 py-20 text-center">
                                    ...
                                    <p class="text-slate-500 font-medium text-sm mt-1"><?= $isOwnScope ? "You haven't processed any service transactions yet." : "No service transactions exist in the system yet." ?></p>
                                </td>
                            </tr>
                        <?php endif; ?>

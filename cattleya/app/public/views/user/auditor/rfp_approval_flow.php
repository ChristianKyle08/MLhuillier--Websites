...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && in_array($flow_action, ['generate_flexible_rfp', 'load_existing_rfp', 'load_multiple_rfps', 'group_multiple_rfps'], true)) {
    ...
    if ($flow_action === 'generate_flexible_rfp') {
        ...
        if (count($rfp_ids) < 2) {
            header('Location: /user/auditor/commission-release?error=' . urlencode('Select at least two RFP requests to group under one requisition slip.'));
            exit;
        }
        ...
        try {
            ...
            header('Location: /user/auditor/commission-release?success=' . urlencode(count($rfp_ids) . ' RFP requests were grouped under requisition slip ' . $master_slip . '.'));
            exit;
        } catch (PDOException $e) {
            header('Location: /user/auditor/commission-release?error=' . urlencode('Failed to group the selected RFP requests.'));
            exit;
        }
    }
}
...
if (!$rfp) {
    echo "<div style='min-height: 100vh; display: flex; align-items: center; justify-content: center; font-family: system-ui, -apple-system, sans-serif; background: linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%);'>
            <div style='text-align: center; background: #fff; padding: 48px 56px; border-radius: 20px; box-shadow: 0 20px 40px -10px rgba(0,0,0,0.08); border: 1px solid #e2e8f0; max-width: 440px;'>
                ...
                <a href='/user/auditor/commission-release' style='display: inline-block; width: 100%; padding: 14px; background: #0f766e; color: #fff; border-radius: 12px; font-weight: 600; text-decoration: none; font-size: 15px; transition: background 0.2s ease;'>Return to Commissions</a>
            </div>
          </div>";
    exit;
}
        ...
        <form id="rfp-workflow-form" method="POST" enctype="multipart/form-data">
            ...
            <div class="cp-actions-bar">
                <div class="action-group-left">
                    <a href="/user/auditor/commission-release" class="btn btn-outline">

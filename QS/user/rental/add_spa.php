...
<head>
    ...
    <title>ML Rental - Add SPA COL</title>
    ...
    <?php 
        if (isset($_POST['spa_btn'])) {
            ...
            if (isset($_FILES['fileUpload']) && $_FILES['fileUpload']['error'] === UPLOAD_ERR_OK) {
                ...
                // Check which attachment slot is empty (from 6 to 15)
                $checkQuery = "SELECT * FROM create_contract WHERE contract_number = '$contractNumber' LIMIT 1";
                ...
                if ($row = mysqli_fetch_assoc($result)) {
                    ...
                    for ($i = 6; $i <= 15; $i++) {
                        if (empty($row["attachment_$i"])) {
                            ...
                            $updateQuery = "
                                ...
                                SET attachment_$i = '$fileContent',
                                    mimeType$i = '$fileType',
                                    attachment_{$i}_filename = '$fileName'
                                WHERE contract_number = '$contractNumber'
                            ";
                            if (mysqli_query($conn, $updateQuery)) {
                                echo "<script>alert('File uploaded successfully to slot $i!');</script>";
                            } else {
                                echo "<script>alert('Error uploading file: " . mysqli_error($conn) . "');</script>";
                            }
                            $slotFound = true;
                            break;
                        }
                    ...
                    if (!$slotFound) {
                        echo "<script>alert('All attachment slots (6–15) are already filled.');</script>";
                    }
                } else {
                    echo "<script>alert('Contract not found.');</script>";
                }
            } else {
                echo "<script>alert('Please select a valid file to upload.');</script>";
            }
        }
        ?>
        ...
        <div class="card shadow-sm border-0 rounded-3">
            <div class="card-header bg-danger text-white d-flex align-items-center">
                ...
                <h6 class="mb-0 text-white">SPA Contract Upload</h6>
            </div>
            <div class="card-body">
                <form action="" method="POST" enctype="multipart/form-data" class="needs-validation" novalidate>
                    ...
                    <div class="mb-3">
                        ...
                        <label id="spa_lbl" class="fw-bold mt-2" style="display:none;"></label>
                    </div>

                    <!-- File Upload -->
                    <div class="mb-3">
                        <label for="fileUpload" class="form-label fw-semibold">
                            <i class="bi bi-upload me-1 text-danger"></i> Upload SPA PDF
                        </label>
                    ...
                    <!-- Submit Button -->
                    <button type="submit" name="spa_btn" id="spa_btn" 
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const notarizeLabel = document.getElementById("spa_lbl");

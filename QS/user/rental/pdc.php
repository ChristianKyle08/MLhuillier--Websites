...
$baseQuery = "
    SELECT id, branch, contract_number, l1_firstname, l1_lastname, request_status, 
           ...
           mainzone, region, area, rfp_status, request_status
...
elseif (in_array($userRole, ['HO','Auditor', 'Finance'])) {
    $conditions = [];
      ...
      <div class="card shadow border-0 rounded-4">
          ...
          <div class="card-header bg-white border-0 rounded-top-4 px-4 py-3 d-flex justify-content-between align-items-center">
              <h5 class="mb-0 fw-bold text-danger d-flex align-items-center">
                  <i class="bi bi-clock-history me-2"></i> For Review Contracts (PDC RFP)
              </h5>
        ...
        <?php
        // --- Dynamic filtering conditions based on user role ---
        $whereZone = $whereRegion = $whereArea = "";

        // Mainzone filter
        if (in_array($userRole, ['HO', 'Auditor', 'Finance'])) {
            $whereZone = ""; // Show all zones
        } else {
        ...
        // --- Apply safe filters ---
        $whereZoneFinal   = buildWhereClause($whereZone, "mainzone <> '' AND mode_of_payment = 'PDC'");
        $whereRegionFinal = buildWhereClause($whereRegion, "region <> '' AND mode_of_payment = 'PDC'");
        $whereAreaFinal   = buildWhereClause($whereArea, "area <> '' AND mode_of_payment = 'PDC'");
        ...
        <!-- MAINZONE FILTER -->
        <?php if (in_array($userRole, ['HO','Auditor','Finance'])): ?>
        <div class="col-md-3">
        ...
        <!-- REGION FILTER -->
        <?php if (in_array($userRole, ['Am-Creator','Vpo-Checker','Vpo-Reviewer','Vpo-Approver','HO','Auditor','Finance'])): ?>
        <div class="col-md-3">
        ...
        <!-- AREA FILTER -->
        <?php if (in_array($userRole, ['Rm-Reviewer','Vpo-Checker','Vpo-Reviewer','Vpo-Approver','HO','Auditor','Finance'])): ?>
        <div class="col-md-3">

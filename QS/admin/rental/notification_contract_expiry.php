...
// ✅ Fetch contracts expiring within 1 year (not terminated & not expired)
  ...
  <?php if ($notifCount > 0): ?>
      ...
      <?php foreach ($groupedData as $mainzone => $data): 
          ...
          foreach ($data['contracts'] as $contract) {
              ...
              $expiryDate = date("M d, Y", strtotime($contract['contract_end']));
              
              $contractList .= "• COL Number: {$colNumber} | Branch: {$branchName} | Expiry: {$expiryDate}\r\n";
          }

          // Construct the professional email body dynamically for this mainzone
          $defaultSubject = "Action Required: Contract of Lease Expiry Notification - " . $mainzone;
          
          $defaultMessage = "Dear {$mainzone} Team,\r\n\r\n"
              . "This is an official notification regarding the upcoming expiration of Contract(s) of Lease (COL) under your supervision. Prompt action is required to ensure uninterrupted operations.\r\n\r\n"
              . "Expiring Contracts (Next 1 to 12 Months):\r\n"
              . $contractList . "\r\n"
              . "Kindly review the listed contract(s) and initiate the necessary steps for renewal, extension, or closure as applicable. Please ensure that all corresponding documents are updated promptly.\r\n\r\n"
                  ...
                  <table class="table table-modern align-middle mb-0">
                      ...
                      <tbody>
                          <?php foreach ($data['contracts'] as $row): 
                              $monthsLeft = (date('Y', strtotime($row['contract_end'])) - date('Y')) * 12 +
                                            (date('m', strtotime($row['contract_end'])) - date('m'));
                                            
                              if ($monthsLeft <= 3) {
                                  $status = '<span class="badge-status bg-danger bg-opacity-10 text-danger border border-danger border-opacity-25">Critical: '.$monthsLeft.' month(s)</span>';
                              } else {
                                  $status = '<span class="badge-status bg-warning bg-opacity-10 text-warning-emphasis border border-warning border-opacity-50">Expiring in '.$monthsLeft.' month(s)</span>';
                              }
                          ?>
                          <tr>
                              ...
                              <td class="fw-medium text-dark"><?= date("M d, Y", strtotime($row['contract_end'])) ?></td>
                              <td><?= $status ?></td>
                          </tr>
                          <?php endforeach; ?>
                      </tbody>
